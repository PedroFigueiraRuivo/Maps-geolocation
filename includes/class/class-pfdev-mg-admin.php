<?php
  class pfdev_maps_geolocation {
      ...
      for ($II = 0; $II < count($arrConfig); $II++) {
        ...
        for ($JJ = 0; $JJ < count($arrConfig[$II]['fields']); $JJ++) {
          add_settings_field(
            ...
            [ $this, $arrConfig[$II]['fields'][$JJ]['callback']],
            $this->plugin_slug . '-admin',
            'settings_section_id_' . ($II + 1),
            [ //args
              'field'         => $arrConfig[$II]['fields'][$JJ]['field'],
              'style'         => $arrConfig[$II]['fields'][$JJ]['style'],
              'class'         => $arrConfig[$II]['fields'][$JJ]['class'],
    ...
    public function pfDev_callback_input($args) {
    ...
    public function pfDev_callback_textarea($args) {
      $value = isset($this->options[$args['base']]) ? (
        esc_attr($this->options[$args['base']])
      ) : ('');
      ...
      for ($II = 0; $II < count($arrConfig); $II++) {
        for ($JJ = 0; $JJ < count($arrConfig[$II]['fields']); $JJ++) {
          if(isset($arrConfig[$II]['fields'][$JJ]['label_under_slug'])){
            ...
            if ($arrConfig[$II]['fields'][$JJ]['field'] === 'textarea') {
              $new_input[ $valueUnderLabel ] = sanitize_textarea_field($input[$valueUnderLabel]);
            } else {
